...
The agent is domain-neutral: it does not know artifact, table, relationship,
or business-rule names. It escalates retrieval only when the current evidence
set does not contain graph-backed evidence and delegates answer generation to
...
class InvestigationAgent:
    """Escalate retrieval and produce an evidence-grounded investigation."""
    ...
    def _needs_investigation(evidence: list[dict[str, Any]]) -> bool:
        """Escalate only when no retrieved item has graph-backed evidence."""
    ...
    def _build_follow_up_queries(query: str) -> list[str]:
        """Create domain-neutral investigation variants from the user query."""
        base = query.strip()
        return [
