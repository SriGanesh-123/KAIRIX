...
class QdrantKnowledgeStore:
    ...
    def connect(self) -> None:
        """Connect to either local or cloud Qdrant.

        Local Qdrant does not require an API key.
        Cloud Qdrant can provide one through QDRANT_API_KEY.
        """
    ...
    def ensure_collection(self, vector_size: int) -> None:
        ...
        if self.client.collection_exists(self.collection_name):
            ...
            if existing_size != vector_size:
                raise ValueError(
                    f"Collection '{self.collection_name}' expects vector size "
                    f"{existing_size}, but embedder produced {vector_size}. "
                    "Use a new collection or matching model."
                )
        ...
        for chunk, vector in zip(chunk_list, vector_list):
            payload = {
                ...
                "source_id": (
                    ...
                    if chunk.get("source_id") is not None
                    else None
                ),
                "artifact_id": (
                    str(chunk.get("artifact_id"))
                    if chunk.get("artifact_id") is not None
                    else None
                ),
    ...
    def _require_client(self) -> None:
        if self.client is None:
            raise RuntimeError(
                "QdrantKnowledgeStore is not connected. "
                "Call connect() first."
            )
